      ...
      <div class="max-w-[1440px] mx-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-16">
          <AnimateOnScroll variant="fadeLeft">
            <h2 class="mb-6" :style="{ fontSize: 'clamp(1.75rem,3vw,2.5rem)', fontWeight: 900, color: RED }">{{ pg.info.title }}</h2>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <AnimateOnScroll v-for="(g, i) in pg.info.guarantees" :key="g" variant="scaleIn" :delay="i * 0.08">
              <div class="p-5 bg-[#F5F5F5] dark:bg-[#111111] border border-black/5 dark:border-white/5 flex flex-col gap-3 h-full">
                <div class="w-9 h-9 flex items-center justify-center" :style="{ background: RED }"><component :is="guaranteeIcons[i]" :size="16" class="text-white" /></div>
        ...
        <div class="flex items-center gap-4 p-5 border border-black/10 dark:border-white/10 bg-[#F5F5F5] dark:bg-[#111111] mb-16">
          <div class="w-10 h-10 flex-shrink-0 flex items-center justify-center" :style="{ background: RED }"><ShieldCheck :size="18" class="text-white" /></div>
    ...
    <section class="py-20 lg:py-28 px-4 sm:px-6 lg:px-10 bg-[#F5F5F5] dark:bg-[#111111]">
      <div class="max-w-[800px] mx-auto">
        <AnimateOnScroll variant="fadeUp" class="mb-10">
          ...
          <h2 class="text-[#111111] dark:text-white mb-3" style="font-size:clamp(1.75rem,3vw,2.25rem);font-weight:900">{{ pg.form.title }}</h2>
          <p class="text-[#666666] dark:text-[#999999]">{{ pg.form.subtitle }}</p>
        </AnimateOnScroll>

        <div v-if="submitted" class="flex flex-col items-start gap-4 py-12 px-8 bg-white dark:bg-[#0A0A0A] border border-black/10 dark:border-white/10">
          <div class="w-12 h-12 flex items-center justify-center" :style="{ background: RED }"><ShieldCheck :size="24" class="text-white" /></div>
          <h3 class="text-[#111111] dark:text-white" style="font-size:1.5rem;font-weight:900">Segnalazione ricevuta</h3>
        ...
        <form v-else class="bg-white dark:bg-[#0A0A0A] p-8 border border-black/10 dark:border-white/10 flex flex-col gap-6" @submit="onSubmit">
          ...
          <div class="flex items-center gap-3 py-4 border-t border-b border-black/10 dark:border-white/10">
            <button type="button" class="relative w-12 h-6 flex-shrink-0 transition-colors" :style="{ background: anonymous ? RED : '#CCCCCC', borderRadius: '999px' }" @click="anonymous = !anonymous">
